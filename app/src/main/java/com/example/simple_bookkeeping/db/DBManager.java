...
import com.example.simple_bookkeeping.toexcel.DemoBean;
import com.example.simple_bookkeeping.utils.FloatUtils;
...
public class DBManager {
    ...
    /*获取某天的支出或者收入的总金额   kind：支出==0    收入===1*/
    ...
    /*获取某月的支出或者收入的总金额   kind：支出==0    收入===1*/
    ...
    /*获取数据库中的所有数据，返回到list中，导出Excel*/
    public static List<DemoBean> getAllToExcel() {
        ...
        while (cursor.moveToNext()) {
            ...
            int month = cursor.getInt(cursor.getColumnIndexOrThrow("month"));
            int day = cursor.getInt(cursor.getColumnIndexOrThrow("day"));
            DemoBean demoBean = new DemoBean(id, typename, sImageId, bz, money, time, year, month, day, kind);
            list.add(demoBean);
        }
        return list;
    }
